  ...
  try {
    if (req.user.role === "customer") {
      ...
      const bookings = await pool.query(`
        SELECT sb.*, v.make, v.model, v.year, v.license_plate, u.name as customer_name
...
router.get("/:id", authorize, async (req, res) => {
  try {
    const booking = await pool.query(`
      SELECT sb.*, v.make, v.model, v.year, v.license_plate, u.name as customer_name, u.email as customer_email, u.phone as customer_phone
      FROM service_bookings sb
      JOIN vehicles v ON sb.vehicle_id = v.id
      JOIN users u ON sb.customer_id = u.id
      WHERE sb.id = $1
