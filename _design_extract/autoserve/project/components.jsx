...
// Page header — used by every non-dashboard page
function PageHeader({ kicker, title, subtitle, actions, meta }) {
  return (
    <header className="border-b border-line pb-5 mb-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          {kicker && <div className="font-mono text-[10px] tracking-[0.22em] text-teal uppercase mb-2">{kicker}</div>}
          <h1 className="text-[26px] font-medium tracking-tight text-ink leading-tight">{title}</h1>
          {subtitle && <p className="text-[13px] text-silver mt-1.5 max-w-2xl">{subtitle}</p>}
        </div>
        {actions && <div className="flex items-center gap-2 shrink-0">{actions}</div>}
      </div>
      {meta && (
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-4 text-[12px] text-silver">
          {meta}
        </div>
      )}
    </header>
...
// Stars — compact rating glyph
function Stars({ value = 5, size = 11 }) {
  return (
    <span className="inline-flex items-center gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Icon key={i} name="star" size={size} className={i <= value ? 'text-warn' : 'text-mute'} strokeWidth={i <= value ? 2 : 1.25} />
      ))}
    </span>
  );
}

// PriorityChip — reused across manager pages
...
Object.assign(window, {
  Icon, StatusBadge, Button, Panel, KPI, PageHeader, Stars, PriorityChip,
